...
def pipeline_test(algo):
    ...
    for i, test_case in enumerate(test_case_posts):
        ...
        if algo == 'basic':
            ...
            top_posts = pipeline(parse_test_case(test_case), parse_test_space(test_space_posts), 
                cleaner, filters, substitutes, weights, alg, test_case["target_count"])
        elif algo == 'tfidf':
            ...
            top_posts = use_similarity(parse_test_case(test_case), encoded_posts, posts, test_case['target_count'])
        elif algo == 'bert':
            load_bert_model()
            encoded_posts = np.load(join(dirname(__file__), '../../encodings/bert/test_space.npy'))
            top_posts = bert_similarity(parse_test_case(test_case), encoded_posts, posts, test_case['target_count'])
        ###
